...
import { Package, Star, ArrowRight, Store, ExternalLink, Download } from "lucide-react";
              ...
              {subscriptions.map((sub) => (
                <div key={sub.id} className="card-elevated rounded-2xl p-6 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {sub.tool.blobUrl && (
                      <a
                        href={`/api/tools/${sub.tool.id}/download`}
                    ...
                    {sub.tool.packageUrl && (
                      <a
                        ...
                        className={`font-semibold px-4 py-2 rounded-full text-sm inline-flex items-center gap-1.5 cursor-pointer ${sub.tool.blobUrl ? "btn-secondary" : "btn-cta"}`}
                      >
                        Access <ExternalLink className="w-3.5 h-3.5" />
                      </a>
                    )}
                    {!sub.tool.blobUrl && !sub.tool.packageUrl && (
                      <Link
